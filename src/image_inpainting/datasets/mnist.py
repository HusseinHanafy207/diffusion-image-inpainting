"""MNIST helpers for inpainting experiments."""
...
def get_mnist_dataset(data_dir: str | Path, *, train: bool = True) -> Dataset:
    """MNIST with ``ToTensor()`` so images are float ``(1, 28, 28)`` in ``[0, 1]``."""
...
def get_mnist_inpainting_datasets(
    data_dir: str | Path,
    ...
    *,
    ...
    """Train and test :class:`InpaintingDataset` wrappers over MNIST."""
    if mask_generator is None:
        mask_generator = MaskGenerator(image_size=28)

    train_ds = InpaintingDataset(
        get_mnist_dataset(data_dir, train=True),
        mask_generator,
        mask_type=mask_type,
        return_label=return_label,
    )
    test_ds = InpaintingDataset(
        get_mnist_dataset(data_dir, train=False),
        mask_generator,
        mask_type=mask_type,
        return_label=return_label,
    )
...
def get_mnist_inpainting_dataloaders(
    batch_size: int,
    data_dir: str | Path,
    mask_generator: MaskGenerator | None = None,
    *,
    ...
    train_ds, test_ds = get_mnist_inpainting_datasets(
        data_dir,
        mask_generator,
        mask_type=mask_type,
    )
